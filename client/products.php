...
<div class="container">
    ...
    if (isset($_GET['id']) && !empty($_GET['id'])) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<a href='product_details.php?id=".$row['id']."' class='btn viewBtn' style='width:110px; height:41px;'>View</a>";
                ...
                echo "<button type='submit' name='add' class='btn addCartBtn' style='width:110px; height:41px;'>Add to Cart</button>";
                echo "</form>";
                echo "</div>";
                echo "</div>";
                echo "</div>";
    ...
    if (isset($_GET['min']) || isset($_GET['max']) || isset($_GET['sort']) || isset($_GET['minram']) || isset($_GET['maxram']) || isset($_GET['minstorage']) || isset($_GET['maxstorage'])) {
        ...
        $sql = "SELECT * FROM products WHERE price >= $min AND price <= $max AND archived=0 AND ram >= $minram AND ram <= $maxram AND storage >= $minstorage AND storage <= $maxstorage";
        ...
        if (isset($_GET['sort'])) {
            ...
            $sql .= ($sort == 'asc') ? " ORDER BY price ASC" : " ORDER BY price DESC";
        }
